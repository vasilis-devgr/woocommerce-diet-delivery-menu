jQuery(document).ready(function ($) {
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.success) {
              ...
              $(".zipcode-msg").html("😊 Μπορείτε να παραγγείλετε");
              ...
              $(".zipcode-msg").html(
                "😞 Δυστυχώς, δεν εξυπηρετούμε ακόμα την περιοχή σας"
              );
              ...
              $.ajax({
                ...
                success: function (response) {
                  ...
                  $(".woopm-product-prices").html(
                    `Κόστος: ` + response.data.kcal_total.total_price
                  );

                  $(".woopm-product-content").remove();
                  if (response.data.description) {
                    $(".woopm-products").after(
                      '<div class="mns-box woopm-product-content">' +
  ...
  $('input[type="radio"].woopm-select-menu').change(function () {
    if ($('input[type="radio"].woopm-select-menu:checked').length) {
      ...
      let availableDay = $(".kcal-select-days").val();
      $.ajax({
        url: woopm_ajax_object.ajax_url,
        type: "POST",
        data: {
  ...
  // menu type change start
  $(".kcal-select-days").on("change", function () {
    let program = $('input[type="radio"].woopm-choose-program:checked').val();
    let menuType = $('input[type="radio"].woopm-select-menu:checked').val();
    let programMenu = $(".kcal-category-select").val();
    let availableDay = $(this).val();
    $.ajax({
      url: woopm_ajax_object.ajax_url,
      type: "POST",
      data: {
  ...
  $(document).on("change", ".kcal-select-days", function () {
    let availableDay = $(this).val();
    let menuType = $('input[type="radio"].woopm-select-menu:checked').val();

    // Only reload products if "make your own menu" is selected
    if (menuType === "own" || menuType === "monthly_menu") {
      let program = $('input[type="radio"].woopm-choose-program:checked').val();
      let programMenu = $(".kcal-category-select").val();
      ...
      $.ajax({
        ...
        success: function (response) {
          $(".woopm-products").html(response.data.content);
          // Don't update price for own menu as it's calculated per product
        },
      });
    }
  });
  ...
  $(document).on("click", ".woopm-add-to-cart-button", function (e) {
    ...
    // Get selected allergens (checkboxes)
    let selectedAllergens = [];
    $('input[type="checkbox"].woopm-select-allergens:checked').each(
      function () {
        if ($(this).val() !== "none") {
    ...
    let currentAllergens =
      selectedAllergens.length > 0 ? selectedAllergens.join(",") : "none";
    let currentAllergensOther = $(".woopm-select-allergens-other").val();

    let totalPrice = $("#total_price").val();

    /*if (!currentAllergens) { 
            alert("Αλλεργιογόνα & ιδιαίτερες προτιμήσεις υποχρεούμαι"); 
            return; 
        }
    ...
    $button.html('<i class="fa fa-spinner fa-spin"></i> Φόρτωση...');
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          Swal.fire({
            title: "Προστέθηκε",
            text: "Το προϊόν είναι στο καλάθι σου",
            icon: "success",
          }).then(() => {
            location.reload();
          });
          ...
          Swal.fire({
            title: "Δεν προστέθηκε",
            text: response.data,
            icon: "error",
            confirmButtonText: "Προσπαθήστε αργότερα",
          });
        }
      },
      error: function () {
        ...
        $button.html("Προσθήκη στο καλάθι");
      },
      complete: function () {
        $button.prop("disabled", false);
        $button.css({
  ...
  $(document).on(
    "change",
    'input[type="checkbox"].woopm-select-allergens',
    function () {
      let $this = $(this);
      let value = $this.val();

      // Handle "none" checkbox - uncheck others if none is selected
      if (value === "none" && $this.is(":checked")) {
        $('input[type="checkbox"].woopm-select-allergens')
          .not($this)
          .prop("checked", false);
        $(".woopm-select-allergens-other-section").hide();
      } else if ($this.is(":checked")) {
        // Uncheck "none" if any other option is selected
        $("#s0").prop("checked", false);
      }
      ...
      if ($("#s6").is(":checked")) {
        ...
        $(".woopm-select-allergens-other-section").hide();
        $(".woopm-select-allergens-other").val("");
      }
    }
  );
  // Allergens end
