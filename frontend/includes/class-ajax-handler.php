...
class WOOPMAjax_Handler
{
    ...
    public function woopm_check_shipping_zone($zipcode)
    {
    ...
    public function woopm_get_product_by_program_type()
    {
        ...
        $show_menus = [];
        if ($available_day == -1) {
            $show_menus = ['Πρωινό', 'Μεσημεριανό', 'Bραδινό'];
        } elseif ($available_day == 1) {
            $show_menus = ['Μεσημεριανό', 'Bραδινό'];
        } elseif ($available_day == 2) {
            $show_menus = ['Μεσημεριανό'];
                                            ...
                                            if (!empty($product_ids)) :
                                                if ($menu_type === 'own') {
                                                    $query = new WP_Query([
                                                        ...
                                                        // keep only products whose menu_price meta exists and isn’t empty
                                                        'meta_query'     => [
                                                            'relation' => 'AND',
                                                ...
                                                if ($query->have_posts()) :
                                                    if (!in_array($meal->name, $show_menus)) {
                                                        continue;
                                                    }

                                                    echo '<h3 class="act-title small" data-meal-type="' . esc_html($meal->name) . '">' . esc_html($meal->name) . '</h3>';
                                                    echo '<div class="d-flex flex-wrap mainly-main" data-meal-type="' . esc_html($meal->name) . '">';
                                                        ...
                                                        <div class="mainly-wd">
                                                            <div class="mainly-bx">
                                                                ...
                                                                <div class="mainly-cnt">
                                                                    <h4><?php the_title(); ?></h4>
                                                                    <p>Θερμίδες μερίδας:
                                                                        <strong><?php echo esc_html($kcal_val); ?> kcal</strong>
                                                                    </p>
                                ...
                                foreach ($meals as $meal) :
                                    ...
                                    if (!empty($product_ids)) {
                                        ...
                                        if ($query->have_posts()) :
                                            ...
                                            echo '<h3 class="act-title" data-meal-type="' . esc_html($meal->name) . '">' . esc_html($meal->name) . '</h3>';
                                            echo '<div class="d-flex flex-wrap mainly-main" data-meal-type="' . esc_html($meal->name) . '">';
        ...
        <?php endif; ?>
        <input type="hidden" name="total_price" id="total_price" value="<?php echo $kcal_total['total_price_plain']; ?>">
<?php

        if ($menu_term) {
            $description = $menu_term->description ? wpautop($menu_term->description) : '';
        }
        $content = ob_get_clean();
    ...
    public function woopm_add_to_cart_product()
    {
        ...
        $shipping_zone = $this->woopm_check_shipping_zone($_POST['zipcode']);

        $total_price = $_POST['total_price'];

        if ($shipping_zone == 2 && $total_price > 30) {

            wp_send_json_error('Η ελάχιστη παραγγελία για αυτόν τον Τ.Κ. είναι 30€.');
        }
    ...
    public function woopm_add_to_cart_single_product()
    {
        ...
        if ($product) {
            ...
            if ($added_to_cart) {
                ...
                wp_send_json_error('Δεν προστέθηκε το προϊόν στο Καλάθι');
            }
        } else {
